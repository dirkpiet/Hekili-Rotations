...
namespace AimsharpWow.Modules
{
    public class SnoogensPVERogueOutlaw : Rotation
    {
        ...
        public override bool CombatTick()
        {
            ...
            if (Aimsharp.IsCustomCodeOn("GrapplingHook") && Aimsharp.SpellCooldown("Grappling Hook") - Aimsharp.GCD() <= 0 && Aimsharp.CustomFunction("IsRMBDown") == 1)
            {
                return false;
            }
            #endregion
            ...
            if (Aimsharp.SpellCooldown("Grappling Hook") - Aimsharp.GCD() > 2000 && GrapplingHook)
            {
                if (Debug)
                {
            ...
            if (GrapplingHook && Aimsharp.CanCast("Grappling Hook", "player", false, true))
            {
            ...
            if (Aimsharp.TargetIsEnemy() && TargetAlive() && TargetInCombat)
            {
                ...
                if (UnitDebuffSap("target") == 0 && UnitDebuffBlind("target") == 0 && !KidneyShot && Wait <= 200)
                {
                    ...
                    if (SpellID1 == 51723 && Aimsharp.CanCast("Fan of Knives", "player", false, true) && MeleeRange && Wait <= 200)
                    {
                        if (Debug)
                        {
                            Aimsharp.PrintMessage("Casting Fan of Knives - " + SpellID1, Color.Purple);
                    ...
                    if (SpellID1 == 13877 && Aimsharp.CanCast("Blade Flurry", "player", false, true) && MeleeRange && Wait <= 200)
                    {
                        if (Debug)
                        {
