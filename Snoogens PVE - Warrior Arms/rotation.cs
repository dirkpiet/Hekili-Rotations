...
{
    public class SnoogensPVEWarriorArms : Rotation
    {
        #region Variables
        string FiveLetters;
        #endregion
        ...
        private void InitializeSpells()
        {
            ...
            foreach (string Spell in m_SpellBook_Arms)
                Spellbook.Add(Spell);
        ...
        public override void Initialize()
        {
            ...
            Aimsharp.PrintMessage("Snoogens PVE - Warrior Arms", Color.Yellow);
        ...
        public override bool CombatTick()
        {
            ...
            if (SpellID1 == 999999 && Aimsharp.HasBuff("Bladestorm", "player", true) && !Bladestorm && Wait <= 200)
            {
            ...
            if (Aimsharp.CanCast("Die by the Sword", "player", false, true))
            {
            ...
            if (Aimsharp.TargetIsEnemy() && TargetAlive() && TargetInCombat)
            {
                ...
                if (Wait <= 200)
                {
                    ...
                    #region Arms Spells - Player GCD
                    ////Player
                    if (SpellID1 == 262161 && Aimsharp.CanCast("Warbreaker", "player", false, true) && MeleeRange)
                    {
                        Aimsharp.Cast("Warbreaker");
                        return true;
                    }

                    if (SpellID1 == 260708 && Aimsharp.CanCast("Sweeping Strikes", "player", false, true) && MeleeRange)
                    {
                        Aimsharp.Cast("Sweeping Strikes");
                        return true;
                    }

                    if (SpellID1 == 262228 && Aimsharp.CanCast("Deadly Calm", "player", false, true) && MeleeRange)
                    {
                        Aimsharp.Cast("Deadly Calm");
                        return true;
                    }

                    if (SpellID1 == 197690 && Aimsharp.CanCast("Defensive Stance", "player", false, true))
                    {
                        Aimsharp.Cast("Defensive Stance");
                        return true;
                    }

                    if (SpellID1 == 845 && Aimsharp.CanCast("Cleave", "player", false, true) && MeleeRange)
                    {
                        Aimsharp.Cast("Cleave");
                        return true;
                    }

                    if (SpellID1 == 152277 && Aimsharp.CanCast("Ravager", "player", false, true))
                    {
                        switch (RavagerCast)
                        {
                            case "Manual":
                    ...
                    #region Arms Spells - Target GCD
                    ////Target
                    if (SpellID1 == 167105 && Aimsharp.CanCast("Colossus Smash", "target", true, true))
                    {
                        Aimsharp.Cast("Colossus Smash");
                        return true;
                    }

                    if (SpellID1 == 260643 && Aimsharp.CanCast("Skullsplitter", "target", true, true))
                    {
                        Aimsharp.Cast("Skullsplitter");
                        return true;
                    }

                    if (SpellID1 == 772 && Aimsharp.CanCast("Rend", "target", true, true))
                    {
                        Aimsharp.Cast("Rend");
                        return true;
                    }

                    if (SpellID1 == 7384 && Aimsharp.CanCast("Overpower", "target", true, true))
                    {
                        Aimsharp.Cast("Overpower");
                        return true;
                    }

                    if (SpellID1 == 12294 && Aimsharp.CanCast("Mortal Strike", "target", true, true))
                    {
